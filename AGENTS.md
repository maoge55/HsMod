...
- 宠物列表目前仅供识别，管理器中的自己/对手宠物下拉框已禁用，不保存本地强制切换值。
- 实机对局验证表明：宠物模型需要服务器创建并下发 `COSMETIC Entity/Card`；只有变体 ID、玩家标签或角落 Spell 不能安全生成模型。
- 不要重新引入本地伪造宠物网络实体的实验代码。除非有完整、可复现且不影响网络状态的实现，否则保持该限制。
